...
from Fred_Data import Fred_Data
...
fred_data = Fred_Data(api_key = api_key)
...
domestic_risk_free_rate_vol = 10 / 100  # volatility for the USD risk-free rate
...
correlation_foreign_asset_and_domestic_risk_free_rate = 10/100
...
plt.ylabel('Cumulative average of \n the simulated option prices')
plt.savefig('cumulative_average.png')

plt.figure(1)
plt.plot(single_time_monte_carlo.timeline, df_domestic_short_rate_processes)
plt.xlabel('Time (years)')
plt.ylabel('Domestic short rate')
plt.savefig('short_rate.png')

plt.figure(2)
plt.plot(single_time_monte_carlo.timeline, df_stock_processes)
plt.xlabel("Time (years)")
plt.ylabel('Quantoed stock price')
