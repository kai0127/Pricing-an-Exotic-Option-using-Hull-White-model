...
class Fred_Data(object):
    ...
    def get_correlation_foreign_asset_and_exchange_rate(self):  # Using historical data during the past 180 days
        ...
        df_nikkei_prices.name = 'nikkei'
        df_jpy_usd.name = 'FX'

        # Trim out the 'NaN' value in the raw data for both processes
        merged_data = pd.merge(df_nikkei_prices, df_jpy_usd, how='outer',left_index=True, right_index = True)
        merged_data = merged_data.dropna()

        list_nikkei_prices = [merged_data.iloc[i]['nikkei'] for i in range(len(merged_data)) ]
        # We have to take reciprocal here, because in our assumption X_t is USD/JPY
        list_usd_jpy = [1/merged_data.iloc[i]['FX'] for i in range(len(merged_data)) ]

        # Compute processes {S(i+1)- S(i) / S(i)} and {X(i+1)- X(i) / X(i)}
        target_process_nikkei = [(list_nikkei_prices[i+1]-list_nikkei_prices[i])/list_nikkei_prices[i] for i in range(len(list_nikkei_prices)-1)]
        target_process_usd_jpy = [(list_usd_jpy[i+1]-list_usd_jpy[i])/list_usd_jpy[i] for i in range(len(list_usd_jpy)-1)]
