#  Copyright (c) 2022/5/5 上午1:15 Last Modified By Kaiwen Zhou
...
class Single_time_Monte_Carlo(object):
    ...
    def start_monte_carlo(self):
        ...
        for i in range(self.iteration_times):
            ...
            epsilon1 = np.random.normal(0, 1)  # standard normal distribution
            epsilon2 = np.random.normal(0, 1)  # another independent standard normal distribution
    ...
    def libor_rate_ratio(self, t, T):
        ...
        return self.bond_data.get_value_on_cubic_spline_for_bond_prices(T) / \
               (
                       self.bond_data.get_value_on_cubic_spline_for_bond_prices(T - self.Delta)
                       - self.bond_data.get_value_on_cubic_spline_for_bond_prices(T)
               ) \
               * (self.p_t_T(t, T, self.get_domestic_risk_free_rate_at_a_specified_time(t)) - 1)
